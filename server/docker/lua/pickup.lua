...
if string.len(cname) > 7 and string.sub(cname, 1, 7) == 'pickup/' then
    ...
    local url = 'http://127.0.0.1:3333/call/incoming';
    ...
    if string.len(res) < 2 then
        api:executeString('uuid_kill ' .. uuid .. ' LOSE_RACE');
    else
        local table = json.decode(res);
        if not table.success then api:executeString('uuid_kill ' .. uuid .. ' LOSE_RACE'); end
    end
end
